...
export default {
  ...
  async  getData(){
   //   let url ='http://127.0.0.1:8000/api/chat/get-chat-by-id/'+this.chat_id
   //   axios
   //   .get(url,
   //   { headers: { 
  ...
  async onSubmit() {
      ...
      // axios.post(
      //  'http://127.0.0.1:8000/api/chat/send-text-message',{message:this.message,chat_id:this.chat_id}, 
      //  {
      //   headers: {
      //   "Content-Type": "application/json",
      //   'Authorization': `Bearer ${localStorage.getItem('access_token')}`,
