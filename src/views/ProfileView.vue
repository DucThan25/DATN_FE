<template>
  <div>
    <div class="profile-banner">
      <div class="profile-banner__cover-image-box">
        <div class="cover-image">
          ...
          <img v-else :src="require('@/assets/images/hinh1.jpg')" alt="">
        ...
        <div class="base-info-box d-flex">
          <div class="avatar-box">
            <img v-if="user?.avatar" :src="user?.avatar" alt="" @click="showAvatar">
    ...
    <div class="profile--content">
      <el-tabs v-model="activeName" type="card" class="one">
          ...
          <div class="profile--content__posts">
            <div class="box__status p-3" v-if="(user?.id) && authUser.data.id === user.id" @click="openDialogCreatePost()">
              ...
              <div class="box__status mt-4 mb-2" v-for="(post, index) in userPost" :key = "index">
                <PostComponent 
                    @eventLikePost="eventLikePost($event)" 
                    @dataPostDetail="openDialogPostDetail($event)"  
                    @postUpdate="editPost($event)" 
                    @postDelete="deletePost($event)" 
                    :post = post>
                </PostComponent>
              </div>
          </div>
          ...
          <div class="list-friend-container">
            <div class="list-friend-header">
              ...
              <div class="list-friend__option">
                <input type="text"
                       placeholder="Nhập tên để tìm kiếm 3"
        ...
        <el-tab-pane name="third" tabindex="2" label="Ảnh">
          <div class="list-friend-container">
            <div class="list-friend-header">
              <h5>Ảnh</h5>
            </div>
            <div>
              <el-tabs >
                <el-tab-pane name="first" tabindex="0" label="Ảnh có mặt bạn">
                  <!-- <img :src="user.cover_image" alt=""> -->
                  <img :src="user.avatar" alt="" :size="10">
                  <img :src="user.cover_image" alt="" :size="10">
            
                </el-tab-pane>
                <el-tab-pane name="second" tabindex="1" label="Ảnh của bạn">
                  <div class="profile--content__posts">
                      <div class="box__status mt-4 mb-2" v-for="(post, index) in userPost" :key = "index">
                        <PostComponent 
                            @eventLikePost="eventLikePost($event)" 
        ...
        <el-tab-pane  tabindex="3" label="Giới thiệu">
          <div class="list-friend-container">
            <!-- <div class="list-friend-header"> -->

              <el-tabs  type="card" :tab-position="tabPosition" style="height: 300px;">
                ...
                <el-tab-pane label="2">
                  <div class="intro-detail mt-3">
                    <p>Nơi từng sống</p>
                    <p v-if="user?.address"><b><i class="el-icon-location-information"></i> </b> {{user?.address}}</p>
                  </div>
    ...
    <el-dialog width="30%" title="Tạo bài viết" :visible.sync="dialogCreatePost">
      ...
      <div class="image__preview" v-if='imageUrl'>
        ...
        <div class="image__preview__option">
          <span @click="addImagePost()"><i class="el-icon-refresh"></i></span>
          <span @click="removeImagePreview()"><i class="el-icon-delete"></i></span>
        </div>
      </div>
    ...
    <el-dialog width="30%" title="Sửa bài viết" :visible.sync="dialogEditPost">
      ...
      <div class="image__preview" v-if='imageUrl'>
        ...
        <div class="image__preview__option">
          <span @click="addImagePost()"><i class="el-icon-refresh"></i></span>
          <span @click="removeImageUpdatePreview()"><i class="el-icon-delete"></i></span>
        </div>
      </div>
      <span slot="footer" class="dialog-footer">
    ...
    <el-dialog 
        width="38%" top="3%"  
        :title="'Bài viết của ' + postDetail?.user?.name"     
        :visible.sync="dialogPostDetail" 
        @close="getInfoUser($route.params.id)">
        <DialogPostDetail 
            @eventLikePost="eventLikePost($event)"  
            :checkMoreOption="checkMoreOption" 
            :postDetail="this.postDetail" 
            @add-comment="handleCreateComment($event)" 
            @eventDeleteComment="handleDeleteComment($event)" 
            @eventUpdateContentComment="handleUpdateComment($event)">
        </DialogPostDetail>
    </el-dialog>
      ...
      <el-form>
        <div class="mb-2">
          ...
          <small id="emailHelp" class="form-text">
            <span class="text-danger">{{infoError.name}}</span>
          </small>
        </div>
        <div class="mb-2">
...
export default {
  ...
  methods: {
    ...
    showAvatar(filename){
      api.showAvatarImage(filename) 
      .then(() =>{
        console.log(filename)

      }).catch(error => {
          console.error('Lỗi khi lấy hình ảnh2:', error);
        });
    },
    updateAvatar()
    {
